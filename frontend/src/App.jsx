...
export default function App() {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      ws.onclose = () => { setConnected(false); timeout = setTimeout(connect, RECONNECT_MS) }
      ws.onerror = () => ws.close()
    }
    connect()
  ...
  // Tick sessions countdown every second
  useEffect(() => {
    const iv = setInterval(() => {
      setSessions(prev => prev.map(s => ({
        ...s,
        remaining_seconds: Math.max(0, (s.remaining_seconds || 0) - 1)
      })))
    }, 1000)
    ...
    <div style={styles.root}>
      <header style={styles.header}>
        <div style={styles.headerLeft}>
          <span style={styles.logo}>ð</span>
          <div>
            <div style={styles.title}>BridgeSpace</div>
            <div style={styles.subtitle}>æ©åºæºè½ç¤¾åéåä¸­å¿ â èªä¸»ç®¡çç³»çµ± v2.0</div>
          </div>
        </div>
      ...
      <footer style={styles.footer}>
        ...
        <span>COM1002 Group 5 Â· HSUHK Â· {new Date().getFullYear()}</span>
      </footer>
    </div>
  )
}
...
function LiveClock() {
  ...
  return <span style={{ color: '#FFF', fontFamily: 'monospace', fontSize: 18 }}>
