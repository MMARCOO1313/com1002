...
SPORT_ICON = {"籃球": "🏀", "排球": "🏐", "羽毛球": "🏸", "乒乓球": "🏓", "匹克球": "🏓"}
# Zones are now fetched dynamically from the API — no hardcoded list.
...
class BridgeSpaceKiosk(tk.Tk):
    ...
    def _get_zone_name(self, zone_id: str) -> str:
        """Fetch zone display name from API, with fallback."""
        zones = api_get_zones()
        for z in zones:
            if z["id"] == zone_id:
                sport = z.get("current_sport", "")
                icon = SPORT_ICON.get(sport, "")
                return f"{z.get('name_zh', zone_id)} {icon} {sport}"
    ...
    def _show_home(self):
        ...
        # Show zone occupancy with current sport
        zones = api_get_zones()
        active_sessions = api_get_active_sessions()
        if zones:
            for z in zones:
                sport = z.get("current_sport", "")
                icon = SPORT_ICON.get(sport, "⚽")
                courts = z.get("courts", 1)
                zone_sessions = [s for s in active_sessions if s.get("zone_id") == z["id"]]
                free = courts - len(zone_sessions)
                color = self.GREEN if free > 0 else self.RED
                row = tk.Frame(p, bg=self.CARD)
                row.pack(fill="x", padx=10, pady=4)
                tk.Label(row, text=f"Zone {z['id']}  {icon} {sport}",
                         font=self.FONT_SMALL, fg=self.WHITE, bg=self.CARD,
                         anchor="w").pack(side="left", padx=12, pady=8)
                tk.Label(row, text=f"{free}/{courts} 場空閒",
    ...
    def _show_zone_select(self):
        ...
        for z in zones:
            ...
            icon = SPORT_ICON.get(sport, "⚽")
            courts = z.get("courts", 1)
            zone_sessions = [s for s in active_sessions if s.get("zone_id") == zid]
            free = courts - len(zone_sessions)
            status = z.get("status", "open")
            dur_min = z.get("session_duration", 2700) // 60

            if free <= 0:
                state = "disabled"
                label = f"Zone {zid} — {z.get('name_zh', '')}  {icon}\n🔴 已滿  {sport} {courts}場  每場{dur_min}分"
                bg = self.GRAY
            elif free < courts:
                state = "normal"
                label = f"Zone {zid} — {z.get('name_zh', '')}  {icon}\n🟡 {free}/{courts} 場空閒  {sport}  每場{dur_min}分"
                bg = self.AMBER
            else:
                state = "normal"
                label = f"Zone {zid} — {z.get('name_zh', '')}  {icon}\n🟢 {free}/{courts} 場空閒  {sport}  每場{dur_min}分"
            ...
            tk.Button(p, text=label, command=lambda _zid=zid: self._join_queue(_zid),
        ...
        try:
            ...
            if walk_in:
                ...
                try:
                    ...
                    sess = api_session_enter(face_id, zone_id)
